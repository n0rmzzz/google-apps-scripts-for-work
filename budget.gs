...
// TODO: Update the calendar ID
const CALENDAR_ID = '[EDIT_THIS]@group.calendar.google.com';
// TODO: Update the spreadsheet URL
const SPREADSHEET_URL = 'https://docs.google.com/spreadsheets/d/[EDIT_THIS]/edit';

// TODO: Set the desired configuraton
const reportPeriodInDays = 120;
const maxEventsToProcess = 1000;

const paymentsCalendar = CalendarApp.getCalendarById(CALENDAR_ID);
// TODO: Mind the convention for calendar entries
const prefixIncome = '[Income] ';
const prefixReminder = '[Reminder] ';

const spreadsheetDocument = SpreadsheetApp.openByUrl(SPREADSHEET_URL);
// TODO: Mind the convention for sheet and column names
...
function processPayments() {
  ...
  const end = new Date(start.getTime() + reportPeriodInDays * DAYS_IN_MILLIS);
...
function processEvent(event) {
  ...
  if (endTime.getTime() - startTime.getTime() > DAYS_IN_MILLIS) {
...
function processEventList(eventList) {
  ...
  validEvents.forEach(event => {
    ...
    if (!monthsVisited.hasOwnProperty(monthStartDate)) {
      ...
      eventsTab.appendRow([`${monthStartDate}-01`, undefined, undefined, monthStartDescription, lastMonthRowNumber]);
      lastMonthRowNumber = eventsTab.getLastRow();
